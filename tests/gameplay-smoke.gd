...
func _run() -> void:
	...
	_check(not main.stage_result.next_stage_card.disabled, "Stage 2 result card becomes selectable after Stage 1 clear")
	_check("AVAILABLE" in main.stage_result.status_label.text, "Stage 2 result card reports that its scene is playable")
