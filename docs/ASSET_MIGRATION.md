...
| `assets/bosses/stage1_foreman/walk_sheet.png` | Approved four-frame heavy walk from `agent/art-production`; normalized in commit `a5a6ee4`; 2x2 cells at 512x512 | Integrated at 6 FPS with shared bottom-center baseline and safe cell margins |
| `assets/bosses/stage1_foreman/sweep_sheet.png` | Approved four-frame sweep attack from `agent/art-production`; normalized in commit `a5a6ee4`; 2x2 cells at 512x512 | Integrated with a frame-2 hammer hitbox and no adjacent-cell fragments |
| `assets/bosses/stage1_foreman/slam_sheet.png` | Approved four-frame ground slam from `agent/art-production`; normalized in commit `a5a6ee4`; 2x2 cells at 512x512 | Integrated with a frame-2 local impact, twin shockwave event, and isolated debris |
| `assets/bosses/stage1_foreman/reaction_sheet.png` | Approved four-pose reaction sheet from `agent/art-production`; normalized in commit `a5a6ee4`; 2x2 cells at 512x512 | Integrated for `hurt` and `defeated`, with `rage` and `stunned` reserved and safe cell margins |
...
| `assets/enemies/zombie_dog/run_sheet.png` | Six-frame generation derived from the approved zombie dog identity anchor; normalized in `agent/art-production` commit `a5a6ee4` after engine atlas validation | User-approved repair; 3×2 sprint loop with adjacent-frame fragments removed, shared bottom-center baseline, and at least 12 px transparent margins |
